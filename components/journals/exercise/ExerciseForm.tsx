...
import ExerciseJournal from "@/components/journals/exercise/ExerciseJournal";
...
export default function ExerciseForm({ selectedDate }: ExerciseFormProps) {
  ...
  const [allExerciseEntries, setAllExerciseEntries] = useState<ExerciseEntry[]>(
    []
  );
  ...
  useEffect(() => {
    try {
      const getExerciseData = async () => {
        ...
        if (res.status === 200) {
          setAllExerciseEntries(res.data);
        }
      };
  ...
  // When selectedDate changes render entries only from selected date
  useEffect(() => {
    setExercises(getSingleDayEntries(selectedDate, allExerciseEntries));
  }, [selectedDate, allExerciseEntries]);
  ...
  const createExerciseEntry = async () => {
    ...
    try {
      ...
      if (validated) {
        ...
        setAllExerciseEntries((prev: ExerciseEntry[]) => [...prev, res.data]);
  ...
  const deleteExerciseEntry = async (id: number) => {
    try {
      ...
      const updatedExercises = allExerciseEntries.filter(
        (exercise) => exercise.id != id
      );

      setAllExerciseEntries(updatedExercises);
    } catch (error) {
  ...
  const updateExerciseEntry = async (
    ...
    try {
      ...
      if (res.status == 200) {
        ...
        setAllExerciseEntries((exercises) =>
