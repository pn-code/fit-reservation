...
export default function NutritionForm({ selectedDate }: NutritionFormProps) {
  ...
  useEffect(() => {
    setFoodEntries(getSingleDayEntries(selectedDate, allFoodEntries));
  }, [selectedDate, allFoodEntries]);
    ...
    try {
      if (validateFoodIntake()) {
        ...
        if (res.status == 200) {
          ...
          setAllFoodEntries((prev: FoodEntry[]) => [...prev, res.data]);
          setName("");
          setCalories(0);
          setCarbs(0);
          setFats(0);
  ...
  const deleteFoodEntry = async (id: number) => {
    try {
      ...
      toast.error(`An error has occurred while attempting to delete ${name}.`);
    }
  };
